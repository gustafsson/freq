...
void main()
{
    ...
    float height     = texture2D(tex_nearest, gl_TexCoord[0].xy).x;
    float heightx     = texture2D(tex_nearest, gl_TexCoord[0].xy + vec2(offset_tex.x,0)).x;
    float heighty     = texture2D(tex_nearest, gl_TexCoord[0].xy + vec2(0,offset_tex.y)).x;
