...
#define STFT_NT_BLOCKSIZE 128
__global__ void kernel_stftNormalizeTransform( cudaPitchedPtrType<float2> input,
                                 unsigned scales )
{
    float2 *p = input.ptr();
    float sum = .0f;
    ...
    __shared__ float data[STFT_NT_BLOCKSIZE];

    for (unsigned a=0; a<=scales/STFT_NT_BLOCKSIZE; ++a)
    {
        unsigned scale = a*STFT_NT_BLOCKSIZE + threadIdx.x;
        if (scale < scales)
        {
            float2 v = p[k*scales + scale];
            data[threadIdx.x] = hypot(v.x,v.y);
        }
        ...
        if (0 == threadIdx.x)
        {
            unsigned N = min(STFT_NT_BLOCKSIZE, scales - a*STFT_NT_BLOCKSIZE);
    ...
    __shared__ float normalize;
    if (0 == threadIdx.x)
        normalize = scales / sum;

    // TODO compute standard deviation and recompute mean using only samples from within 1 sigma.
        ...
        unsigned scales )
{
    ...
    dim3 block(STFT_NT_BLOCKSIZE);
    dim3 grid(height);

    kernel_stftNormalizeTransform<<<grid, block, 0>>>( in, scales );
}
